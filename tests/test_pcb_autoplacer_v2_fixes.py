...
@pytest.mark.unit
class TestSpringClassesProjectIO:
    """Read + bootstrap of mcp_spring_classes in .kicad_pro.  Uses a
    tmp_path-backed JSON file rather than a real KiCad project."""

    def _write_pro(self, tmp_path, data):
        import json
        pro = tmp_path / "test.kicad_pro"
        pro.write_text(json.dumps(data))
        return pro

    def test_bootstrap_creates_section_when_missing(self, tmp_path):
        import json
        from commands.pcb_autoplacer import (
            _bootstrap_spring_classes_in_project,
        )
        pro = self._write_pro(tmp_path, {"existing_field": "value"})
        modified = _bootstrap_spring_classes_in_project(pro)
        assert modified
        data = json.loads(pro.read_text())
        assert "mcp_spring_classes" in data
        assert data["mcp_constraint_version"] == 2
        assert "classes" in data["mcp_spring_classes"]
        assert "DECOUPLING" in data["mcp_spring_classes"]["classes"]
        assert data["mcp_spring_classes"]["classes"]["DECOUPLING"]["spring_k"] == 5.0
        assert data["existing_field"] == "value"  # didn't trample

    def test_bootstrap_idempotent_preserves_user_edits(self, tmp_path):
        import json
        from commands.pcb_autoplacer import (
            _bootstrap_spring_classes_in_project,
        )
        # User has tweaked DECOUPLING + added a net assignment.
        pro = self._write_pro(tmp_path, {
            "mcp_constraint_version": 2,
            "mcp_spring_classes": {
                "classes": {
                    "DECOUPLING": {"spring_k": 7.0, "natural_length": 0.0},
                },
                "nets": {"USB_VBUS": "PLANE"},
            },
        })
        ...
        assert not modified
        data = json.loads(pro.read_text())
        assert data["mcp_spring_classes"]["classes"]["DECOUPLING"]["spring_k"] == 7.0
        assert data["mcp_spring_classes"]["nets"]["USB_VBUS"] == "PLANE"

    def test_load_classes_merges_with_defaults(self, tmp_path):
        from commands.pcb_autoplacer import (
        ...
        pro = self._write_pro(tmp_path, {
            "mcp_spring_classes": {
                "classes": {
                    # Override DECOUPLING; add a new BULK_DECOUPLING class.
                    "DECOUPLING": {"spring_k": 8.0, "natural_length": 0.0},
                    "BULK_DECOUPLING": {"spring_k": 2.0, "natural_length": 3.0},
                },
                "nets": {"USB_VBUS": "PLANE", "V12_OUT": "INTER_GROUP"},
            },
        })
        classes, nets = _load_spring_classes_from_project(pro)
        assert classes["DECOUPLING"].spring_k == 8.0
        assert classes["BULK_DECOUPLING"].spring_k == 2.0
        assert classes["BULK_DECOUPLING"].natural_length == 3.0
        # Defaults still present for un-overridden classes.
        assert classes["LOCAL_SIGNAL"].spring_k == 1.0
        assert classes["PLANE"].spring_k == 0.0
        assert nets == {"USB_VBUS": "PLANE", "V12_OUT": "INTER_GROUP"}

    def test_load_handles_missing_section(self, tmp_path):
        from commands.pcb_autoplacer import (
            _load_spring_classes_from_project,
        )
        pro = self._write_pro(tmp_path, {"unrelated": "data"})
        classes, nets = _load_spring_classes_from_project(pro)
        # Falls back to defaults; no net assignments.
        assert classes["DECOUPLING"].spring_k == 5.0
        assert nets == {}

    def test_load_skips_bad_entries_without_crashing(self, tmp_path):
        from commands.pcb_autoplacer import (
            _load_spring_classes_from_project,
        )
        pro = self._write_pro(tmp_path, {
            "mcp_spring_classes": {
                "classes": {
                    "GOOD": {"spring_k": 2.0},
                    "BAD_NUMERIC": {"spring_k": "not a number"},
                    42: {"spring_k": 1.0},
                },
                "nets": {
                    "OK_NET": "GOOD",
        ...
        classes, nets = _load_spring_classes_from_project(pro)
        assert classes["GOOD"].spring_k == 2.0
        assert "BAD_NUMERIC" not in classes
        assert nets == {"OK_NET": "GOOD"}
