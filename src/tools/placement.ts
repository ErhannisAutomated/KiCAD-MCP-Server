...
export function registerPlacementTools(server: McpServer, callKicadScript: Function) {
  ...
  server.tool(
    "pre_route_audit",
    "Pre-flight terminal-based feasibility check at each net's OWN netclass widths. The Phase 4+5 workflow tool — run this BEFORE the autoroute attempt to flag impossible connections with actionable remediation hints. Read-only. **Terminal model**: a terminal is a pad OR a zone; both are first-class members of a net. For each net the audit picks the largest-area terminal as the **hub** (a same-net plane pour dominates over the pads it serves) and asks 'can each other terminal reach the hub?' via the multi-layer meta-graph. Output is per-spoke, with `hub` and `spoke` carrying `{uid, kind, label, areaMm2, layers}`. This collapses to 'does every pad have via access to the plane?' for plane-connected nets — the engineering question — and catches unstitched bridge zones natively: a same-net pour that can't reach the main hub shows up as an `spoke_unreachable_from_hub` with `clusterPeers` listing the other terminals stuck with it. Per-net analysis is used when a net has zones (so the pour is visible to via-candidacy); shared all-copper analysis otherwise. Returns `{summary, spokes, netclassesEvaluated, limitations}`. Reason codes: `spoke_no_anchor`, `zone_no_anchor`, `spoke_unreachable_from_hub`, `pads_on_different_nets`. Pass `*Override` to force one width/via/clearance across all nets (what-if surveys).",
    {
