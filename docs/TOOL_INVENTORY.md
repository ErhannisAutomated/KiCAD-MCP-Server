...
| `pre_route_audit`         | Pre-flight terminal-based feasibility at each net's OWN netclass widths. Hub = largest-area terminal (pad OR zone) per net; spokes = every other terminal; per-net analysis when same-net zones exist. Catches unstitched bridge zones natively. Phase 4+5 of [TOPOLOGY_TOOLS_PLAN](TOPOLOGY_TOOLS_PLAN.md). Read-only. | Additional |
